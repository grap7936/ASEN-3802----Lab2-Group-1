...
'Brass',115,...
'Steel',16.2);
dataFolder = 'C:\Users\Graeme\Documents\MATLAB\ASEN3802_HeatConduction_FA25';
...
% (references embedded code from Part 1 Task 2)
...
alpha_adj = 0; % Preallocate to an arbitrary value
...
% Assign original thermal diffusivity to different material values
...
%mod2
y_ana2_SS = T0_Cases + Han(i)*x_TH;
y_ana2 = sum_rows_ana2 + y_ana2_SS;

% ** Model III
y_ana_adj_SS = T0_Cases + Han(i)*x_TH;
y_ana_adj = sum_ana_adj + y_ana_adj_SS;
...
figure(17)
subplot(2,3,i)
y_analytical_IB = sum_experimental + Hexp(i)*x_TH + T0_Cases;
...
% Plot -- Model III with adjusted thermal diffusivity
figure(19)
subplot(2,3,i)
...
figure(20)
subplot(2,3,i)
plot(t, Accuracy_RMSE, "b", "LineWidth", 1.5);
hold on
title(file_string(i))
xlabel("Time Elapsed [s]")
ylabel('Point by point accuracy');
